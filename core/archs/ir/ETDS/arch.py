...
class ETDSForInferencenew(nn.Module):
    ''' ETDS for inference arch with depthwise separable convolutions '''
    def __init__(self, num_in_ch, num_out_ch, upscale, num_block, num_feat, num_residual_feat):
        super().__init__()
        assert (num_residual_feat >= num_in_ch) and (num_out_ch == num_in_ch)
        self.upscale = upscale
        ...
        self.conv_first = nn.Conv2d(num_in_ch, num_feat, kernel_size=3, padding=1, groups=num_in_ch)

        # Depthwise separable convolutions for the backbone
        backbone_convs = []
        for _ in range(num_block):
            backbone_convs.append(nn.Conv2d(num_feat, num_feat, kernel_size=3, padding=1, groups=num_feat))
        self.backbone_convs = nn.ModuleList(backbone_convs)

        # Handling last convolution with separable convolutions
        if upscale == 3:
            self.conv_last = nn.Conv2d(num_feat, num_out_ch * (upscale**2) + 1, kernel_size=3, padding=1, groups=num_feat)
            self.conv_clip = nn.Conv2d(num_out_ch * (upscale**2) + 1, num_out_ch * (upscale**2) + 1, 1, groups=num_out_ch * (upscale**2) + 1)
        else:
            self.conv_last = nn.Conv2d(num_feat, num_out_ch * (upscale**2), kernel_size=3, padding=1, groups=num_feat)
            self.conv_clip = nn.Conv2d(num_out_ch * (upscale**2), num_out_ch * (upscale**2), 1, groups=num_out_ch * (upscale**2))
    ...
    def forward(self, input):
        ...
        # If upscale is 3, apply the conv_clip operation, else just use conv_clip without modification
        if self.upscale == 3:
            x = F.relu(self.conv_clip(x))[:, :-1, :, :]
        else:
            x = F.relu(self.conv_clip(x))
